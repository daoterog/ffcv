...
def validate_simple_dataset(name, length, shuffled=False):
    ...
    if shuffled:
        assert_that(
            (reader.metadata["f0"] == np.arange(length).astype("int")).all()
        ).is_false()
        assert_that(set(reader.metadata["f0"])).is_equal_to(
            set(np.arange(length).astype("int"))
        )
    else:
        assert_that(
            (reader.metadata["f0"] == np.arange(length).astype("int")).all()
    ...
    assert_that(
        (np.sin(reader.metadata["f0"]) == reader.metadata["f1"]).all()
    ).is_true()
...
def test_write_shuffle():
    ...
    with NamedTemporaryFile() as handle:
        ...
        writer = DatasetWriter(name, {"index": IntField(), "value": FloatField()})
...
def test_multiple_workers():
    ...
    with NamedTemporaryFile() as handle:
        ...
        writer = DatasetWriter(
            name,
            {"index": IntField(), "value": FloatField()},
...
def test_small_chunks_multiple_workers():
    ...
    with NamedTemporaryFile() as handle:
        ...
        writer = DatasetWriter(
            name,
            {"index": IntField(), "value": BytesField()},
            num_workers=min(30, cpu_count()),
        )
