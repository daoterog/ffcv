...
UNAUGMENTED_PIPELINE = [SimpleRGBImageDecoder(), ToTensor(), ToTorchImage()]
...
def run_test(length, pipeline, should_compile=False, aug_name=""):
    ...
    with NamedTemporaryFile() as handle:
        ...
        tot_indices = 0
        tot_images = 0
        for it_num, ((images, labels), (original_images, original_labels)) in enumerate(
            zip(loader, unaugmented_loader)
        ):
            ...
            if SAVE_IMAGES and it_num == 0:
                save_image(
                    make_grid(
                        ch.concat([images, original_images]) / 255.0, images.shape[0]
                    ),
                    os.path.join(
                        IMAGES_TMP_PATH, aug_name + "-" + str(uuid.uuid4()) + ".jpeg"
                    ),
                )
...
def test_cutout():
    for comp in [True, False]:
        run_test(
            100,
            [SimpleRGBImageDecoder(), Cutout(8), ToTensor(), ToTorchImage()],
            comp,
            "cutout",
        )
...
def test_random_resized_crop():
    for comp in [True, False]:
        run_test(
            ...
            [
                SimpleRGBImageDecoder(),
                RandomResizedCrop(scale=(0.08, 1.0), ratio=(0.75, 4 / 3), size=32),
                ToTensor(),
                ToTorchImage(),
            ],
            comp,
