...
def create_and_validate(length, decoder, size, mode="raw", compile=False):
    ...
    with NamedTemporaryFile() as handle:
        ...
        fields = {"index": IntField(), "value": RGBImageField(write_mode=mode)}
        ...
        loader = Loader(
            ...
            batch_size=5,
            num_workers=2,
            pipelines={"value": [decoder, ToTensor()]},
        )
...
def test_simple_image_decoder_fails_with_variable_images():
    ...
    assert_that(create_and_validate).raises(TypeError).when_called_with(
        500, decoder, 32, "raw"
    )
