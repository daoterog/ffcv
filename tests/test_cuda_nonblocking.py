...
class DummyArrayDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return (
            (np.random.rand(SIZE) > 0.5).astype("bool"),
            np.random.rand(SIZE).astype("float32"),
            index,
        )
...
def test_cuda():
    ...
    assert ch.abs(sync_1 - sync_2).max().cpu().item() < float(
        WORKERS
    ), "Sync-sync mismatch"
    assert ch.abs(async_1 - sync_1).max().cpu().item() < float(
        WORKERS
    ), "Async-sync mismatch"
