...
def find_pkg_dirs(package):
    close_matches = get_close_matches(
        package.lower(), os.environ["PATH"].lower().split(";"), cutoff=0
    )
    dll_dir = None
    for close_match in close_matches:
        if os.path.exists(close_match) and glob(os.path.join(close_match, "*.dll")):
            dll_dir = close_match
            break
    if dll_dir is None:
        raise Exception(
...
def pkgconfig_windows(package, kw):
    ...
    while parent != root_dir:
        ...
        library_dirs = set()
        libraries = glob(
            os.path.join(parent, "**", "lib", "**", "*.lib"), recursive=True
        )
        for library in libraries:
            if (is_x64 and "x86" in library) or (not is_x64 and "x64" in library):
                continue
...
extension_kwargs = {"sources": sources, "include_dirs": []}
...
libffcv = Extension("ffcv._libffcv", **extension_kwargs)
