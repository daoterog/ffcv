...
class RandomResizedCrop(Operation):
    ...
    def __init__(
        self, scale: Tuple[float, float], ratio: Tuple[float, float], size: int
    ):
        super().__init__()
        self.scale = scale
        self.ratio = ratio
    ...
    def generate_code(self) -> Callable:
        ...
        def random_resized_crop(images, dst):
            for idx in my_range(images.shape[0]):
                i, j, h, w = fast_crop.get_random_crop(
                    images[idx].shape[0], images[idx].shape[1], scale, ratio
                )
    ...
    def declare_state_and_memory(
        ...
        return replace(
            previous_state, jit_mode=True, shape=(self.size, self.size, 3)
