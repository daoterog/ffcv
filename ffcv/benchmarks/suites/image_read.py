...
class ImageReadBench(Benchmark):
    ...
    def __init__(
        self, n, length, mode, size, random_reads, compile, num_workers, batch_size
    ):
        self.n = n
        self.mode = mode
        self.length = length
    ...
    def __enter__(self):
        ...
        writer = DatasetWriter(
            self.length,
            name,
            {"index": IntField(), "value": RGBImageField(write_mode=self.mode)},
        )
        ...
        def code(indices, buff, state):
            ...
            for i in range(0, len(indices), self.batch_size):
                result += decode(
                    indices[i : i + self.batch_size], buff, reader.metadata["f1"], state
                )[0, 5, 5]
            return result
