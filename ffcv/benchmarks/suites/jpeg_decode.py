...
class JPEGDecodeBenchmark(Benchmark):
    ...
    def __enter__(self):
        ...
        resized_image = cv2.resize(
            loaded_image, (new_width, new_height), interpolation=cv2.INTER_AREA
        )
        _, self.encoded_image = cv2.imencode(
            ".jpg", resized_image, [int(cv2.IMWRITE_JPEG_QUALITY), self.quality]
        )
