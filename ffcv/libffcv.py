import ctypes
import platform
from ctypes import (CDLL, POINTER, c_bool, c_int64, c_uint8, c_uint32,
...
def imdecode(
    ...
    offset_y=0,
    scale_factor_num=1,
    scale_factor_denom=1,
    enable_crop=False,
    do_flip=False,
):
    return ctypes_imdecode(
        source.ctypes.data,
        source.size,
        source_height,
        source_width,
...
def memcpy(source: np.ndarray, dest: np.ndarray):
    return ctypes_memcopy(
        source.ctypes.data, dest.ctypes.data, source.size * source.itemsize
    )
